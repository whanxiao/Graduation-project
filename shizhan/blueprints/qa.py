...
@bp.route("/ice_box")
def bx_zl():
    return render_template("Refrigerator/bx_zl.html")

@bp.route("/milk_powder")
def milk_zl():
    return render_template("milk powder/milk_zl.html")

@bp.route("/lxz")
def lxz_zl():
    return render_template("lxz/lxz_zl.html")
...
def public_question(name):
    ...
    if request.method == 'GET':
        ...
        if form.validate():
            ...
            flash("请输入最少6个字的内容！！")
            return redirect(url_for("qa.public_question",name=name))
